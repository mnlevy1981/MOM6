...
subroutine ocn_import(x2o, ind, grid, ice_ocean_boundary, ocean_public, logunit, Eclock, c1, c2, c3, c4)
  ...
  do j = jsc, jec
    do i = isc, iec
      ...
      !       currently the conversion is done in MARBL_tracers:MARBL_tracers_column_physics when copying into surface_flux_forcings
      !       but maybe we should convert somewhere else because we want to register these IOB fields as diagnostics (in g/cm^2/s for now)
