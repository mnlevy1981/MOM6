...
function get_extern_field_axes(field) result(axes)
  ...
  if (rc /= NF90_NOERR) &
    call MOM_err(FATAL, "Error opening file " // trim(field%filename) // ".")

  ! Use field%label to get the netCDF varid
  rc = nf90_inq_varid(ncid, trim(field%label), varid)
  if (rc /= NF90_NOERR) &
    call MOM_err(FATAL, "Error finding variable " // trim(field%label) &
  ...
  if (rc /= NF90_NOERR) &
    call MOM_err(FATAL, "Error querying variable " // trim(field%label) &
        // " in " // trim(field%filename) // ".")

  if (ndims < 3) &
    call MOM_err(FATAL, trim(field%label) // " in " // trim(field%filename) &
  ...
  do d=1,ndims
    ...
    if (rc /= NF90_NOERR) then
      write(d_str, '(i0)') d
      call MOM_err(FATAL, "Error querying dimension " // trim(d_str) &
    ...
    if (rc /= NF90_NOERR) &
      call MOM_err(FATAL, "Error finding dimension variable " &
    ...
    if (rc /= NF90_NOERR) &
      call MOM_err(FATAL, "Error reading dimension " // trim(dim_name) &
  ...
  rc = nf90_close(ncid)
  if (rc /= NF90_NOERR) &
    call MOM_err(FATAL, "Error closing file "//trim(field%filename)//".")
